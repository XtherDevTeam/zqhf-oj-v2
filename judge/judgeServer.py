import flask, json, config, requests, io, pickle,os,sys,time
...
def execute_plugin( use_plugin:str, source_file:str, input:str, env:dict, time_out:int = 1000, memlimit:int = 1024):
...
def submit_():
    ...
    pickleData.close()
    result_data = execute_plugin(data['plugin'], data['source_file'], data['input'], data['env_variables'], data['time_limit'], data['mem_limit'])
    return checker({
